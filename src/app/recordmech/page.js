...
import {
  Calendar, Clock, User, RefreshCw, Menu,
  LayoutGrid, List, Stethoscope, CalendarCheck,
  X, Check,
} from 'lucide-react';
...
export default function MechAppointmentRecords() {
  ...
  const deletePrescription = async (appointmentId) => {
    if (!confirm("確定要刪除此處方箋嗎？")) return;
    try {
      const res = await fetch(`/api/delete-prescription/${appointmentId}`, {
        method: "DELETE",
        credentials: "include",
      });
      const data = await res.json();
  ...
  // ── 處方箋區塊（卡片用） ──────────────────────────────────────────
  const PrescriptionSection = ({ appointment }) => (
    <div className="mt-4 border-t pt-4">
      <p className="text-sm font-semibold text-gray-700 mb-3">處方箋檔案</p>
      ...
      {appointment.prescription_image ? (
          ...
          <div className="flex flex-wrap gap-2 justify-center">
            ...
            <label className="px-4 py-2 bg-yellow-500 hover:bg-yellow-600 text-white rounded-lg text-sm transition cursor-pointer">
              ...
              <input
                ...
                onChange={(e) => {
                  if (e.target.files[0])
                    uploadPrescription(appointment.appointment_id, e.target.files[0]);
                }}
              />
            </label>

            <button
              onClick={() => deletePrescription(appointment.appointment_id)}
        ...
        <label className="flex flex-col items-center justify-center border-2 border-dashed border-blue-300 rounded-2xl p-10 bg-blue-50 hover:bg-blue-100 transition cursor-pointer">
          ...
          <p className="text-sm font-medium text-blue-700">點擊上傳處方箋</p>
          <p className="text-xs text-blue-500 mt-1">支援 JPG、PNG、PDF</p>
          <input
            type="file"
            accept="image/*,.pdf"
            className="hidden"
        ...
        <div className="flex-1 p-6">
          ...
          <div className="flex gap-1 bg-white rounded-xl shadow-sm border border-gray-200 p-1 mb-6 w-fit">
            ...
            >
              ...
              回診審核
              {/* 紅點提示 */}
              {followUpRequests.filter((r) => r.status === "pending_review").length > 0 &&
                pageTab !== "followup" && (
                      ...
                      <tbody className="divide-y divide-gray-200">
                        {filteredAppointments.map((appointment) => (
                          <tr key={appointment.appointment_id} className="hover:bg-gray-50 transition">
                            ...
                            <td className="px-6 py-4">
                              <span className="text-green-700 font-medium text-sm">{appointment.doctor.specialty}</span>
                            </td>
                            ...
                            <td className="px-6 py-4 text-sm max-w-xs">
                              ...
                              {appointment.status === '已完成' && (
                                <span className="text-blue-600">{appointment.doctor_advice || '尚未填寫'}</span>
                              )}
          ...
          {pageTab === "followup" && (
            <>
              ...
              <div className="bg-white rounded-lg shadow-md p-4 mb-6 flex flex-wrap items-center gap-3">
                ...
                <button
                  onClick={fetchFollowUpRequests}
                  className="ml-auto flex items-center gap-1.5 px-3 py-2 text-sm text-gray-600 hover:bg-gray-100 rounded-lg transition"
                >
                    ...
                    return (
                      <div
                        ...
                        className={`bg-white rounded-xl shadow-md overflow-hidden border-l-4 ${
                          isPendingReview ? "border-orange-400" : "border-gray-200"
                        }`}
                      >
                        <div className="p-5">
                          {/* 狀態 + 申請日期 */}
        ...
        <div className="fixed inset-0 bg-black/40 flex items-center justify-center z-50 px-4">
          <div className="bg-white rounded-xl shadow-2xl w-full max-w-md border border-gray-200">
            ...
            <div className="p-5 space-y-4">
              <div className="bg-gray-50 rounded-lg p-3 text-sm">
                ...
                <p className="font-semibold text-gray-800">
                  {rejectTarget.selected_date} {rejectTarget.selected_time?.slice(0, 5)}
                </p>
              </div>
              <div>
            ...
            <div className="p-5 border-t border-gray-200 flex gap-3">
              ...
              >
                {isProcessing
                  ? <><RefreshCw size={15} className="animate-spin" /> 處理中...</>
                  : <><X size={15} /> 確定拒絕</>
                }
              </button>
            </div>
