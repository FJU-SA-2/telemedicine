  ...
  return (
    <div className="bg-white rounded-xl shadow-md hover:shadow-2xl transition-all border border-gray-100 overflow-hidden cursor-pointer transform hover:-translate-y-1 duration-300">
      ...
      <div className="p-4 sm:p-5">
        <div className="flex items-start gap-3 sm:gap-4 mb-4" onClick={() => onPatientClick(patient)}>
          <div className={`w-14 h-14 sm:w-16 sm:h-16 bg-gradient-to-br ${utils.getGenderColor(patient.gender)} rounded-full flex items-center justify-center text-white font-bold text-xl sm:text-2xl shadow-lg flex-shrink-0`}>
          ...
          <div className="flex-1 min-w-0">
            ...
            <p className="text-xs sm:text-sm text-gray-500">病歷號：#{patient.patient_id}</p>
        ...
        <div className="space-y-2 mb-4 pb-4 border-b border-gray-100" onClick={() => onPatientClick(patient)}>
          ...
          {patient.address && (
            <div className="flex items-start gap-2 text-xs sm:text-sm text-gray-600">
              <MapPin size={15} className="text-red-600 flex-shrink-0 mt-0.5" />
        ...
        <div className="grid grid-cols-2 gap-3 mb-4">
          <div className="bg-blue-50 rounded-lg p-3 border border-blue-100">
            ...
            <p className="text-base sm:text-lg font-bold text-blue-600">{patient.total_appointments || 0}</p>
          </div>
          <div className="bg-green-50 rounded-lg p-3 border border-green-100">
            <p className="text-xs text-gray-600 mb-1">最近就診</p>
            <p className="text-xs font-semibold text-green-600">
              {patient.last_appointment_date
                ? new Date(patient.last_appointment_date).toLocaleDateString('zh-TW', { month: 'short', day: 'numeric' })
                : "無紀錄"}
            </p>
          </div>
        </div>
