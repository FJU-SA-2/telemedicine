...
export async function GET(request) {
  ...
  try {
    ...
    if (!meRes.ok) return NextResponse.json({ message: "請先登入" }, { status: 401 });
    ...
    if (!doctor_id) return NextResponse.json({ message: "找不到醫師資料" }, { status: 404 });
...
>>>>>>> b2134d2cef361e6ce607955a752363a180571ba2
    ...
    return NextResponse.json({ message: `取得歷史記錄失敗: ${error.message}` }, { status: 500 });
  } finally {
